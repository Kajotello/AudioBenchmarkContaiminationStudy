    ...
    def forward(
        ...
        if exists(media_locations):
            ...
            for batch_idx in range(B):
                media_locations_b = media_locations[
                    batch_idx
                ].nonzero()  # locations of <audio>
                ...
                for i in range(-1, len(media_locations_b)):
                    ...
                    few_shot_mask[
                        batch_idx,
                        text_start:text_end,
                        look_at_window_start:look_at_window_end,
                    ] = True

            sim = sim.masked_fill(
                ~few_shot_mask.unsqueeze(1), -torch.finfo(sim.dtype).max
            )
