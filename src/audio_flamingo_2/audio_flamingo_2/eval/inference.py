...
def inference_this(
    ...
    if test_dataset_name in data_config["valid_dataset_config"]:
        data_config["valid_dataset_config"] = {
            test_dataset_name: data_config["valid_dataset_config"][test_dataset_name]
        }
    else:
        data_config["valid_dataset_config"] = {test_dataset_name: True}

    all_test_AudioTextDataInfo = get_audiotext_dataloader(
        data_config, clap_config, tokenizer, args.batch_size, split="test"
    )

    assert test_dataset_name in list(
        all_test_AudioTextDataInfo.keys()
    ), "{} not a test set".format(test_dataset_name)
    ...
    if os.path.exists(tmp_file):
        ...
        print(
            "reading tmp data from {}: {} batches already computed".format(
                tmp_file, finished_batches + 1
            )
        )
        ...
        for idx in range(input_ids.shape[0]):
            ...
            if deduplicate:
                if (filename, prompt_decoded) in results_dic:
                    results_dic[(filename, prompt_decoded)]["ground_truth"].append(
                        ground_truth_decoded
                    )
                ...
                results.append(
                    (filename, prompt_decoded, ground_truth_decoded, output_decoded)
                )
...
def main():
    ...
    parser.add_argument(
        "-temp", "--temperature", type=float, default=1.0, help="temperature"
    )
    parser.add_argument(
        "-nb", "--num_beams", type=int, default=1, help="num beams for beam search"
    )
    parser.add_argument(
        "--ckpt", type=int, default=-1, help="checkpoint idx, -1 means latest"
    )
    ...
    output_file = os.path.join(
        ...
        "{}-ckpt{}.log".format(
            parsed_args.config.split("/")[-1][:-5], parsed_args.ckpt
        ),
    )
