...
class DataCollator:
    ...
    def __call__(self, batch):
        ...
        filenames, audio_clips, audio_embed_masks, input_ids, attention_masks = zip(
            *batch
        )

        num_windows_all = [
            sum(audio_embed_mask) for audio_embed_mask in audio_embed_masks
        ]
        ...
        for audio_clip, audio_embed_mask in zip(audio_clips, audio_embed_masks):
            ...
            if num_windows < max_window_batch:
                for _ in range(max_window_batch - num_windows):
                    this_audio_clip_clips.append(
                        torch.zeros_like(this_audio_clip_clips[-1])
                    )
        ...
        audio_embed_mask = torch.cat(
            [x.unsqueeze(0) for x in padded_audio_embed_masks], dim=0
        )
...
class AudioTextData(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if self.split == "train":
            self.data = self.blend_dataset(
                dataset_blending_config, dataset_blending_output
            )

        elif self.split in ["val", "test"]:
            self.valid_data = self.validation_dataset(
                valid_dataset_config, valid_dataset_name
            )
    ...
    def blend_dataset(self, dataset_blending_config, dataset_blending_output):
        if os.path.exists(dataset_blending_output) and not self.force_reblend:
            ...
            if not self.force_reblend:
                ...
                print(
                    "force reblending dataset at epoch {}; reading all dataset files".format(
                        self.epoch
                    )
                )
            ...
            for dataset_name in dataset_blending_config:
                dataset_file = os.path.join(
                    self.dataset_file_root, "{}.json".format(dataset_name)
                )
                contents = self._read_dataset_file(dataset_file)
                contents["data"] = self.shuffle_dict_fixed_rand(
                    contents["data"], seed=sum(list(map(ord, dataset_name)))
                )
                ...
                all_data[dataset_name] = {"contents": contents, "weight": weight}
            ...
            for dataset_name in all_data:
                ...
                for idx in range(start_idx, end_idx):
                    if idx > 0 and idx % dataset_total_num == 0:
                        print(
                            "force shuffling at new epoch {} for dataset {}".format(
                                idx // dataset_total_num, dataset_name
                            )
                        )
                            ...
                            seed=sum(
                                list(
                                    map(
                                        ord,
                                        "{}-epoch-{}".format(
                                            dataset_name, idx // dataset_total_num
                                        ),
                                    )
                                )
                            ),
            ...
            if not self.force_reblend:
                ...
                print(
                    "writing reblended dataset file to:",
                    dataset_blending_output.replace(".json", "-reblended.json"),
                )
                with open(
                    dataset_blending_output.replace(".json", "-reblended.json"), "w"
                ) as json_file:
                    json.dump(self_data, json_file)
    ...
    def get_num_windows(self, T, sr):
        ...
        if T <= window_length:
            ...
            full_length = (
                num_windows * window_length - (num_windows - 1) * window_overlap
            )
    ...
    def compute_sliding_window(self, audio_file, audio_start=0.0, audio="sound"):
        ...
        duration = (
            max_num_window
            * (encoder_config["window_length"] - encoder_config["window_overlap"])
            + encoder_config["window_overlap"]
        )

        audio_data = self.load_audio(
            os.path.join(self.data_root, audio_file), sr, duration, audio_start
        )  # already cuts to max duration
        T = len(audio_data)
    ...
    def validation_dataset(self, valid_dataset_config, valid_dataset_name):
        dataset_file = os.path.join(
            self.dataset_file_root, "{}.json".format(valid_dataset_name)
        )
        contents = self._read_dataset_file(dataset_file)

        contents["data"] = self.shuffle_dict_fixed_rand(
            contents["data"], seed=sum(list(map(ord, valid_dataset_name)))
        )
    ...
    def _actual_getitem(self, i):
        if self.split == "train":
            ...
            if type(item["name"]) is str:
                ...
                audio_start = (
                    0 if "audio_start" not in item else float(item["audio_start"])
                )
            else:
                raise Exception(
                    f"The item has a {type(item['name'])}. Only single path as a string is supported"
                )

            # compute window for long audios
            audio_clips, audio_embed_mask = self.compute_sliding_window(
                audio_file, audio_start, audio="sound"
            )
