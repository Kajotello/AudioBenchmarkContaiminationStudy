...
def predict_with_context(
    filepath1, prompt1, answer1, filepath2, prompt2, clap_config, inference_kwargs
):
    ...
    audio_clips = audio_clips.unsqueeze(0).to(
        device_id, dtype=cast_dtype, non_blocking=True
    )
    audio_embed_mask = audio_embed_mask.unsqueeze(0).to(
        device_id, dtype=cast_dtype, non_blocking=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Sample index from CLAPv2/Clotho validation split",
    )
    parser.add_argument(
        "--prompt",
        "-p",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to audio file; if set, skips dataset loading",
    )
    # Few-shot / context args
    parser.add_argument(
        "--audio1", type=str, default=None, help="Path to context audio (example 1)"
    )
    parser.add_argument(
        "--prompt1",
        type=str,
        default=None,
        help="Question for context audio (example 1)",
    )
    parser.add_argument(
        "--answ1", type=str, default=None, help="Answer for context audio (example 1)"
    )
    parser.add_argument(
        "--audio2", type=str, default=None, help="Path to query audio (example 2)"
    )
    parser.add_argument(
        "--prompt2", type=str, default=None, help="Question for query audio (example 2)"
    )
    ...
    if all(a is not None for a in few_shot_args):
        ...
        sample = dataset[parsed_args.idx]
        ...
        try:
            predict(tmp_path, parsed_args.prompt, clap_config, inference_kwargs)
        finally:
            os.unlink(tmp_path)
