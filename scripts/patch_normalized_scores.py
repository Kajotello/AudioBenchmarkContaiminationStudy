...
def _patch_metrics_file(metrics_path: Path, member_val: float, non_member_val: float, dry_run: bool) -> bool:
    ...
    existing_keys = {line.split(":")[0].strip() for line in text.splitlines() if ":" in line}
...
def _patch_wandb(entity: str, project: str, run_id: str, member_val: float, non_member_val: float, dry_run: bool) -> str:
    ...
    if "member_normalized_score" not in run.summary or run.summary["member_normalized_score"] is None:
        updates["member_normalized_score"] = member_val
    if "non_member_normalized_score" not in run.summary or run.summary["non_member_normalized_score"] is None:
...
def main() -> None:
    ...
    p.add_argument("--logs-dir", type=Path, default=Path("logs/contamination/runs"),
    ...
    if not csv_files:
        print(f"No codec_per_sample_results.csv found under {args.logs_dir}", file=sys.stderr)
        sys.exit(0)

    print(f"Found {len(csv_files)} run(s) to process.{' [DRY-RUN]' if args.dry_run else ''}")
    ...
    for csv_path in csv_files:
        ...
        print(f"  {label}: member_norm={member_val:.4f}  non_member_norm={non_member_val:.4f}")

        metrics_path = run_dir / "codec_metrics.txt"
        file_changed = _patch_metrics_file(metrics_path, member_val, non_member_val, args.dry_run)
        if file_changed:
            patched_local += 1
        ...
        if run_id:
            wb_status = _patch_wandb(args.entity, args.project, run_id, member_val, non_member_val, args.dry_run)
            if wb_status == _WANDB_UPDATED:
                patched_wandb += 1
                print(f"    wandb run {run_id} {'[would update]' if args.dry_run else 'updated'}")
