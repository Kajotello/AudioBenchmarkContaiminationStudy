...
ax.set_title('CoDeC Normalized Scores  (filled = Member, hollow = Non-Member)',
...
for i, ds in enumerate(DATASETS):
    ...
    for j, model in enumerate(MODELS):
        ...
        if has_member and member_score is not None:
            ax.scatter(x0 + M_OFF, member_score,
                       marker=MARKERS[j], color=COLORS[j],
                       s=MARKER_SIZE, zorder=3)

        ax.scatter(nm_x, non_member_score,
                   marker=MARKERS[j], s=MARKER_SIZE, zorder=3,
                   facecolors='none', edgecolors=COLORS[j], linewidths=1.8)

    # Dataset label + M/NM sub-labels
    label_y = -0.06
    ax.text(x0 + BLOCK / 2, label_y, ds, ha='center', va='top',
            fontsize=10, transform=ax.get_xaxis_transform())
    if has_member:
        ax.text(x0 + M_OFF, label_y - 0.04, 'M', ha='center', va='top',
                fontsize=8, color='steelblue', transform=ax.get_xaxis_transform())
    ax.text(nm_x, label_y - 0.04, 'NM', ha='center', va='top',
...
legend_models = ax.legend(handles=model_handles, title='Model', loc='upper left',
                           framealpha=0.8, fontsize=9, title_fontsize=9)
ax.add_artist(legend_models)

# Member/Non-member legend (right)
member_patch    = mpatches.Patch(facecolor='grey', label='Member (filled)')
nonmember_patch = mpatches.Patch(facecolor='white', edgecolor='grey',
                                  linewidth=1.5, label='Non-Member (hollow)')
ax.legend(handles=[member_patch, nonmember_patch], loc='upper right',
...
ax2.set_title('CoDeC Normalized Scores — Member vs Non-Member per Model', fontsize=13, fontweight='bold', pad=10)
...
for i, model in enumerate(MODELS):
    ...
    nm_scores = [v for ds, (m, nm) in DATA[model].items() if nm is not None for v in [nm]]
...
def style_boxes(bp, color):
    ...
    plt.setp(bp.get('fliers', []), marker='o', markerfacecolor=color,
             markeredgecolor=color, markersize=5)

bp_m  = ax2.boxplot(box_data_m,  positions=box_positions_m,  widths=0.22,
                    patch_artist=True, notch=False,
                    manage_ticks=False)
bp_nm = ax2.boxplot(box_data_nm, positions=box_positions_nm, widths=0.22,
                    patch_artist=True, notch=False,
                    manage_ticks=False)
...
member_patch    = mpatches.Patch(facecolor=member_color,    alpha=0.5, label='Member (3 datasets)')
nonmember_patch = mpatches.Patch(facecolor=nonmember_color, alpha=0.5, label='Non-Member (5 datasets)')
